...
class InstagramProfileWorksWindow(SimpleToolWindow):
    ...
    def tool_config_params(self):
        return [
            ...
            ConfigParam("scroll_interval", "滚动间隔(秒)", kind="float", default=3.0, minimum=0.5, maximum=10.0, step=0.1, decimals=1),
            ConfigParam("scroll_px", "每次滚动像素(px)", kind="int", default=2600, minimum=500, maximum=10000, step=100),
            ...
            ConfigParam("cooldown_max", "冷却等待最大(秒)", kind="float", default=25.0, minimum=0.0, maximum=120.0, step=1.0, decimals=1),
    ...
    def run_task(self, values, log_callback, finish_callback, stop_event, pause_event):
        ...
        config = {k: v for k, v in values.items() if k in ("max_works", "page_load_timeout", "scroll_interval", "scroll_px", "no_new_scroll_limit", "max_scrolls", "save_batch_size", "cooldown_min", "cooldown_max", "detail_delay_min", "detail_delay_max")}
